...
class L1NNTauAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit L1NNTauAnalyzer(const edm::ParameterSet&);
  ~L1NNTauAnalyzer();
...
L1NNTauAnalyzer::L1NNTauAnalyzer(const edm::ParameterSet& iConfig)
...
L1NNTauAnalyzer::~L1NNTauAnalyzer()
{
...
void
L1NNTauAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  Initialize();

  if(debug_){
    std::cout<<" Starting L1NNTau Analyzer ............     "<< std::endl;  
  }
    using namespace edm;
   ...
   for(auto l1PFTau : *l1PFTauHandle){
     ...
     */
     if(debug_){
       std::cout<<" L1NNTau pt "<<l1PFTau.pt()<<" eta "<< l1PFTau.eta()<<" phi "<< l1PFTau.phi()<<" charge "<< l1PFTau.charge()<<" Type "<< 1 <<" chargedIso "<< l1PFTau.chargedIso()<<std::endl;
       std::cout<<" L1NNTau Z " << l1PFTauZ  << std::endl;
     }
     /*
...
void L1NNTauAnalyzer::Initialize() {
  indexevents_ = 0;
  runNumber_ = 0;
  lumi_ = 0;
  MC_weight_ = 1;
...
DEFINE_FWK_MODULE(L1NNTauAnalyzer);
