...
L1HPSPFTauAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(debug_){
   ...
   edm::Handle<std::vector<l1t::TkPrimaryVertex>> l1VertexHandle;
   ...
   for(auto l1PFTau : *l1PFTauHandle){
     ...
     */
     ...
     if(l1PFTau.pt()!=0){
       //std::cout<<" Iso "<<l1PFTau.sumChargedIso()<<" Pt "<<l1PFTau.pt()<<" Iso/Pt "<<l1PFTau.sumChargedIso()/l1PFTau.pt()<<std::endl;
       l1PFTauIsoOverPt_.push_back(l1PFTau.sumChargedIso()/l1PFTau.pt());
     }
     ...
     for(unsigned int i=0; i<l1VertexHandle->size(); i++){
       ...
       double temp_dz = abs(l1PFTauZ - l1VertexHandle->at(i).zvertex());
	 ...
	 if((l1VertexHandle->size()!=0) && abs(l1PFTau.signalAllL1PFCandidates().at(i)->pfTrack()->vertex().z() - l1VertexHandle->at(l1PFTauVtxIndex).zvertex()) <= l1PFTaudz){
     ...
     if(applyBDT_){
       /*
       bdt_l1PFTauPt_ = l1PFTau.pt();
       bdt_l1PFTauEta_ = l1PFTau.eta();
       bdt_l1PFTauIso_ = l1PFTau.sumChargedIso();
       bdt_l1PFTauNeutralIso_ = l1PFTau.sumNeutralIso();
       ...
       //bdt_l1PFTauHoverE_ = l1PFTauHoverE;
       bdt_l1PFTauSignalTrackMass_ = l1PFTauSignalTrack_p4.mass();
       Float_t BDT = tmva_reader_->EvaluateMVA("BDTG");
       */

       const std::map<std::string, double> bdtInputsValues = {
	 //{"l1PFTauPt",                 l1PFTau.pt()},
	 ...
	 {"l1PFTauSignalTrackMass",    l1PFTauSignalTrack_p4.mass()}
       ...
       const double BDT = (*bdt_tmva_L1HPSPFTau)(bdtInputsValues);
       //const double BDT_xgb = (*bdt_xgb_L1HPSPFTau)(bdtInputsValues);
...
L1HPSPFTauAnalyzer::beginJob()
{
  ...
  if(applyBDT_){
    ...
    */
    ...
    //std::string fileName_tmva = "L1TauAnalyzerPhase2/L1TauAnalyzerPhase2/data/L1HPSPFTau_XGB_testVars_default_7Var.xml";
